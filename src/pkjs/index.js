...
'<div class="row"><label for="bg">Background</label><input type="color" id="bg" value="#ffffff"></div>',
'<div class="row"><label for="line">Line</label><input type="color" id="line" value="#000000"></div>',
...
'<div class="row"><label for="showmarkers">Show time markers</label><input type="checkbox" id="showmarkers" style="width:24px;height:24px"></div>',
'<div class="row"><label for="showhourdots">Show hour dots</label><input type="checkbox" id="showhourdots" checked style="width:24px;height:24px"></div>',
'<div class="row" id="extendrow"><label for="extendlines">Extend second lines past screen edge</label><input type="checkbox" id="extendlines" checked style="width:24px;height:24px"></div>',
'<div class="row" style="flex-wrap:wrap"><label style="width:100%;margin-bottom:8px">Hour lines start at <span id="hourpctval">25</span>% radius</label><input type="range" id="hourpct" min="0" max="90" step="1" value="25" style="width:100%"></div>',
'<div class="row" style="flex-wrap:wrap"><label style="width:100%;margin-bottom:8px">Minute lines start at <span id="minpctval">55</span>% radius</label><input type="range" id="minpct" min="0" max="90" step="1" value="55" style="width:100%"></div>',
'<div class="row" style="flex-wrap:wrap"><label style="width:100%;margin-bottom:8px">Second lines start at <span id="secpctval">80</span>% radius</label><input type="range" id="secpct" min="0" max="95" step="1" value="80" style="width:100%"></div>',
...
'var saved=(typeof __S!=="undefined")?__S:{};if(saved.bg_color)document.getElementById("bg").value=saved.bg_color;if(saved.line_color)document.getElementById("line").value=saved.line_color;if(saved.smooth_threshold!=null){smoothEl.value=saved.smooth_threshold;svalEl.textContent=saved.smooth_threshold;}if(saved.seconds_threshold!=null){secEl.value=saved.seconds_threshold;secvalEl.textContent=saved.seconds_threshold;}if(saved.battery_show_threshold!=null){batEl.value=saved.battery_show_threshold;batvalEl.textContent=saved.battery_show_threshold;}if(saved.line_rotation!=null){document.getElementById("linerot").checked=!!saved.line_rotation;}if(saved.show_date!=null){document.getElementById("showdate").checked=!!saved.show_date;}if(saved.show_markers!=null){markersEl.checked=!!saved.show_markers;}if(saved.show_hour_dots!=null){document.getElementById("showhourdots").checked=!!saved.show_hour_dots;}if(saved.extend_second_lines!=null){extendEl.checked=!!saved.extend_second_lines;}if(saved.hour_start_pct!=null){hourPctEl.value=saved.hour_start_pct;hourPctValEl.textContent=saved.hour_start_pct;}if(saved.minute_start_pct!=null){minPctEl.value=saved.minute_start_pct;minPctValEl.textContent=saved.minute_start_pct;}if(saved.second_start_pct!=null){secPctEl.value=saved.second_start_pct;secPctValEl.textContent=saved.second_start_pct;}updateExtendVisibility();',
'document.getElementById("save").addEventListener("click",function(){var s={bg_color:document.getElementById("bg").value,line_color:document.getElementById("line").value,smooth_threshold:parseInt(smoothEl.value,10),seconds_threshold:parseInt(secEl.value,10),battery_show_threshold:parseInt(batEl.value,10),line_rotation:document.getElementById("linerot").checked?1:0,show_date:document.getElementById("showdate").checked?1:0,show_markers:markersEl.checked?1:0,show_hour_dots:document.getElementById("showhourdots").checked?1:0,extend_second_lines:extendEl.checked?1:0,hour_start_pct:parseInt(hourPctEl.value,10),minute_start_pct:parseInt(minPctEl.value,10),second_start_pct:parseInt(secPctEl.value,10)};document.location=returnTo+encodeURIComponent(JSON.stringify(s));});',
'})();</script></body></html>'
].join('');
...
Pebble.addEventListener('showConfiguration', function () {
  if (!Object.keys(s_settings).length) {
...
Pebble.addEventListener('webviewclosed', function (e) {
  ...
  s_settings = settings;
  try { localStorage.setItem('rlf_pkjs_settings', JSON.stringify(settings)); } catch(e) {}
  var msg = {
  ...
  if (settings.show_hour_dots != null) {
    msg.SHOW_HOUR_DOTS = settings.show_hour_dots;
  }
