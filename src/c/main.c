...
static GPoint hour_pivot(GPoint center, int32_t r, int hour) {
  ...
  return (GPoint){
    .x = center.x + sin_lookup(hour_angle) * (r / 6) / TRIG_MAX_RATIO,
    .y = center.y - cos_lookup(hour_angle) * (r / 6) / TRIG_MAX_RATIO,
  };
}
...
static void draw_clock_lines(GContext *ctx, GPoint center, int32_t r, int32_t angle) {
  for (int hour = 0; hour < 12; hour++) {
    ...
    draw_rotating_line(ctx, hour_pivot(center, r, hour), r / 3, angle + offset);
  }
}
